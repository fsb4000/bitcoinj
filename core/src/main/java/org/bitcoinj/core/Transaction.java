...
import static org.bitcoinj.params.Networks.Family.NOVACOIN;
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    public Transaction(NetworkParameters params) {
        ...
        if (txFamily == PEERCOIN || txFamily == NOVACOIN || txFamily == NUBITS || (txFamily == CANNACOIN && version > 1) ||
    ...
    protected int calcLength(byte[] buf, int offset) {
        ...
        if (Networks.isFamily(params, PEERCOIN, NUBITS, RUBYCOIN, BLACKCOIN, NOVACOIN))
    ...
    public boolean isCoinStake() {
        if (Networks.isFamily(params, PEERCOIN, NUBITS, RUBYCOIN, CANNACOIN, BLACKCOIN, REDDCOIN, NOVACOIN)) {
            maybeParse();
    ...
    protected void bitcoinSerializeToStream(OutputStream stream, boolean includeExtensions) throws IOException {
        ...
        if (Networks.isFamily(params, PEERCOIN, NUBITS, RUBYCOIN, BLACKCOIN, NOVACOIN) && includeExtensions)
